...
normalization = 0; % 1 for normalization, 0 for raw data
...
subplot(3,4,1)
plot(tempPhi,plotPhiSus,'.')
title('$\sigma_t(\Phi)$','Interpreter','Latex')
box on
grid on
grid minor
axis square
...
subplot(3,4,5)
plot(tempPhi,stdPhiSus,'.')
title('$\sigma_J(\sigma_t(\Phi))$','Interpreter','Latex')
box on
grid on
grid minor
axis square
...
subplot(3,4,6)
plot(tempPhi,stdPhi,'.')
title('$\sigma_J(\Phi)$','Interpreter','Latex')
box on
grid on
grid minor
axis square
...
subplot(3,4,7)
plot(temp,stdSus,'.')
title('$\sigma_J(\chi)$','Interpreter','Latex')
box on
grid on
grid minor
axis square
...
subplot(3,4,8)
plot(temp,stdM,'.')
title('$\sigma_J(M)$','Interpreter','Latex')
box on
grid on
grid minor
axis square
...
bl = boundedline(tempPhi, mean(plotPhiSus,2), stdPhiSus,'.-', ...
    'cmap', colors, ...
    'alpha');
title('$\bar{\sigma_t}(\Phi)$','Interpreter','Latex')
xticks([])
box on
grid on
grid minor
...
subplot(3,4,12)
b3 = boundedline(temp, mean(plotM,2), stdM, '.-', ...
    'cmap', colors, ...
    'alpha');
...
set(bl,'MarkerSize',10)
set(b2,'MarkerSize',10)
set(b3,'MarkerSize',10)

if normalization
    
    for iSubplot = 9:12
        subplot(3,4,i) % normalize yaxis to be between 0 and 1
        ylim([0 1])
    end
...
figure(1)
suptitle(['Random Networks, Metropolis Transitions, n = ', num2str(length(data))])
% suptitle('Random Networks, Glauber Transitions, n = 28')
suplabel('Temperature','x');
...
%         'alpha');
    ...
    lh = legend(bl);
    legnames = {'\chi : Susceptibility', '\phi : Integrated Information', '\sigma(\Phi): std.'};
...
Tmin_RAND = [];
pKS_RAND = [];
for iter = 1:size(Corr_DTI,4) % random sims loop
    iter
    ...
    for iNet = 1:length(Networks)
        ...
        [ks2stat_RAND(iNet,iter,:), pKS_RAND(iNet,iter,:), PD_RAND(iNet,iter,:)] = ks_test_dist(Corr_DTI(:,:,:,iter),Corr_FMRI(Ind,Ind));
        
        [~,InDM] = min(PD_RAND(iNet,iter,:));
        Tmin_RAND(iNet,iter) = temp(InDM);
...
for iNet = 1:9
    ...
    title([Networks{iNet}, ', p = ', num2str(p)])
...
for iNet = 1:length(Networks)
    ...
    switch plotCase
        ...
        case 'phi'
            ...
            hold all
            h2 = plot(temp,Phi,'.','Color',colors(2,:),'MarkerSize',mkSize,'DisplayName','\Phi');
            
            if drawPhiSus
                h6 = plot(temp,PhiSus,'.','Color',colors(8,:),'MarkerSize',mkSize,'DisplayName','\sigma(\Phi)');
            end
            
            if drawLine
            %%%%%%%%%%% Tmin Line %%%%%%%%%%%
            IndC = find(abs(temp - Out.Tmin_NET(iNet)) < 0.001);
            line([temp(IndC),temp(IndC)],[0,Sus(IndC)],'Color','b')
            str = sprintf('$$T_{min} = %.2g $$',temp(IndC));
            text('Interpreter','latex','Position',[temp(IndC+1) 1/10],'String',str,'FontSize',14)
            %%%%%%%%%%% Tc Line %%%%%%%%%%%
            IndC = find(abs(temp - Out.Tc_NET(iNet)) < 0.001);
            line([temp(IndC),temp(IndC)],[0,Sus(IndC)],'Color','r')
            str = sprintf('$$T_{c} = %.2g $$',temp(IndC));
            text('Interpreter','latex','Position',[temp(IndC+1) 3/10],'String',str,'FontSize',14)
            if drawPhiSus
                %%%%%%%%%%% Tcphi Line %%%%%%%%%%%
                ...
                text('Interpreter','latex','Position',[temp(IndC+1) 5/10],'String',str,'FontSize',14)
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            end
            end
